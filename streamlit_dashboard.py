...
# streamlit_dashboard.py (regenerated with guard)
import io
import pandas as pd
import streamlit as st
from pathlib import Path  # Path used for default sample

# ---- Plotly import guard (friendly error if missing) ----
try:
    import plotly.graph_objects as go
    import plotly.express as px
    from plotly.subplots import make_subplots
except ModuleNotFoundError:
    st.error("Plotly가 설치되지 않았습니다. 터미널에서 `pip install plotly` 또는 "
             "`pip install -r requirements.txt` 실행 후 앱을 다시 시작하세요.")
    st.stop()
